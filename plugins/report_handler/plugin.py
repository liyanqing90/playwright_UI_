...
class HTMLReportHandler(ReportHandler):
    ...
    def _generate_html_content(self, results: List[TestResult], config: ReportConfig) -> str:
        """生成HTML内容"""
        # 统计信息
        total = len(results)
...
<body>
    ...
    def _generate_test_items_html(self, results: List[TestResult], config: ReportConfig) -> str:
        """生成测试项HTML"""
        items_html = []
...
class ReportHandlerPlugin:
    ...
    def _register_commands(self):
        ...
        CommandFactory.register(["attach_screenshot", "添加截图"])(AttachScreenshotCommand)
        CommandFactory.register(["attach_page_source", "添加页面源码"])(AttachPageSourceCommand)
        CommandFactory.register(["add_test_result", "添加测试结果"])(AddTestResultCommand)
    ...
    def generate_report(self, format_name: str, config: Optional[ReportConfig] = None) -> str:
        """生成报告"""
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if 'start_time' in result_data and isinstance(result_data['start_time'], str):
                result_data['start_time'] = datetime.datetime.fromisoformat(result_data['start_time'])
            if 'end_time' in result_data and isinstance(result_data['end_time'], str):
                result_data['end_time'] = datetime.datetime.fromisoformat(result_data['end_time'])

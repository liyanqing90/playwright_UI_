...
class ExpressionEvaluatorPlugin:
    ...
    def _register_commands(self):
        """注册插件命令"""
        # 注册表达式计算命令
        CommandFactory.register(["evaluate_expression", "计算表达式"])(EvaluateExpressionCommand)
        CommandFactory.register(["register_function", "注册函数"])(RegisterFunctionCommand)
    ...
    def _register_builtin_functions(self):
        ...
        self.register_function("replace", lambda s, old, new: s.replace(old, new), "替换字符串", "string", 3)
        ...
        # 条件函数
        self.register_function("if_", lambda condition, true_val, false_val: true_val if condition else false_val, "条件判断", "conditional", 3)
    
    def register_function(self, name: str, func: Callable, description: str = "", category: str = "custom", args_count: Optional[int] = None):
        """注册函数"""
    ...
    def evaluate(self, expression: str, context: Optional[Dict[str, Any]] = None, variable_manager=None) -> Any:
        """评估表达式"""
        try:
            # 检查表达式长度
            if len(expression) > self.config['max_expression_length']:
                raise ValueError(f"表达式长度超过限制: {len(expression)} > {self.config['max_expression_length']}")
...
class RegisterFunctionCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            # 注册函数
            plugin.register_function(name, custom_function, description, category, args_count)

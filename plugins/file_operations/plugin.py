...
class ReadFileCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 存储到变量
            if variable_name:
                ui_helper.store_variable(variable_name, content, step.get('scope', 'global'))
            ...
            if format_type == 'json':
                ...
                with open(abs_path, mode, encoding=encoding, newline='') as f:
                    if isinstance(content, list) and content:
                        fieldnames = content[0].keys() if isinstance(content[0], dict) else None
                        if fieldnames:
...
class ListDirectoryCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 存储到变量
            if variable_name:
                ui_helper.store_variable(variable_name, file_list, step.get('scope', 'global'))
...
class GetFileInfoCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 存储到变量
            if variable_name:
                ui_helper.store_variable(variable_name, info_dict, step.get('scope', 'global'))
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if variable_name and backup_path:
                ui_helper.store_variable(variable_name, backup_path, step.get('scope', 'global'))
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if format_type == 'zip':
                ...
                shutil.make_archive(base_name, format_type, os.path.dirname(abs_source), os.path.basename(abs_source))
            
            # 存储压缩文件路径到变量
            if variable_name:
                ui_helper.store_variable(variable_name, abs_archive, step.get('scope', 'global'))
...
class ExtractFileCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 存储解压路径到变量
            if variable_name:
                ui_helper.store_variable(variable_name, abs_extract, step.get('scope', 'global'))
...
class SearchFilesCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # 存储到变量
            if variable_name:
                ui_helper.store_variable(variable_name, result, step.get('scope', 'global'))
            ...
            for i, operation in enumerate(operations):
                try:
                    ...
                    if op_type in command_map:
                        ...
                        results.append({'index': i, 'status': 'success', 'operation': operation})
                    else:
                        raise ValueError(f"不支持的操作类型: {op_type}")
                        
                except Exception as e:
                    error_info = {'index': i, 'status': 'error', 'operation': operation, 'error': str(e)}

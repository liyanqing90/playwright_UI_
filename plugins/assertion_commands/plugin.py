...
class AssertionManager:
    ...
    def execute_assertion(
        self, assertion_id: str, actual_value: Any, config: AssertionConfig
    ) -> AssertionResult:
        """执行断言"""
        ...
        try:
            ...
            if config.assertion_type == AssertionType.RETRY:
                result = self._execute_retry_assertion(
                    assertion_id, processed_actual, processed_expected, config
                )
            elif config.assertion_type == AssertionType.TIMEOUT:
                result = self._execute_timeout_assertion(
                    assertion_id, processed_actual, processed_expected, config
                )
            else:
                result = self._execute_basic_assertion(
                    assertion_id, processed_actual, processed_expected, config
                )
    ...
    def _execute_basic_assertion(
        self, assertion_id: str, actual: Any, expected: Any, config: AssertionConfig
    ) -> AssertionResult:
        """执行基础断言"""
        passed = False
        message = ""
        ...
        try:
            ...
            if config.custom_validator:
                ...
                passed, message = self._evaluate_assertion(
                    actual, expected, config.operator
                )
    ...
    def _execute_retry_assertion(
        ...
        for attempt in range(config.retry_count + 1):
            try:
                result = self._execute_basic_assertion(
                    assertion_id, actual, expected, config
                )
                ...
                if result.passed:
                    if attempt > 0:
                        logger.info(
                            f"重试断言成功: {assertion_id}, 尝试次数: {attempt + 1}"
                        )
                    return result
    ...
    def _execute_timeout_assertion(
        self, assertion_id: str, actual: Any, expected: Any, config: AssertionConfig
    ) -> AssertionResult:
        """执行超时断言"""
        start_time = time.time()
        ...
        if last_result:
            last_result.message = (
                f"超时断言失败: {last_result.message} (超时: {elapsed:.2f}s)"
            )

        return last_result

    def _evaluate_assertion(
        self, actual: Any, expected: Any, operator: AssertionOperator
    ) -> tuple[bool, str]:
        """评估断言"""
        try:
            if operator == AssertionOperator.EQUALS:
                ...
                message = (
                    f"期望 '{actual}' 长度等于 {expected}, 实际长度: {actual_length}"
                )
                ...
                passed = actual_length > int(expected)
                message = (
                    f"期望 '{actual}' 长度大于 {expected}, 实际长度: {actual_length}"
                )
                ...
                passed = actual_length < int(expected)
                message = (
                    f"期望 '{actual}' 长度小于 {expected}, 实际长度: {actual_length}"
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats["total_assertions"] > 0:
            stats["pass_rate"] = (
                stats["passed_assertions"] / stats["total_assertions"] * 100
            )
            stats["fail_rate"] = (
                stats["failed_assertions"] / stats["total_assertions"] * 100
            )
        else:
            stats["pass_rate"] = 0
            stats["fail_rate"] = 0
    ...
    def get_failed_assertions(
        self, assertion_type: Optional[AssertionType] = None
    ...
    def check_soft_assertions(self) -> bool:
        ...
        if self.soft_assertion_failures:
            ...
            raise AssertionError(
                f"软断言失败 ({len(self.soft_assertion_failures)} 个): {'; '.join(failure_messages)}"
            )
        return True
...
class BaseAssertCommand(Command):
    ...
    def create_assertion_config(
        ...
        return AssertionConfig(
            assertion_type=AssertionType(
                step.get("assertion_type", assertion_type.value)
            ),
            operator=operator,
...
class AssertEqualsCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_equals_{int(time.time() * 1000)}"
        )
        ...
        result = self.assertion_manager.execute_assertion(
            assertion_id, actual_value, config
        )

        if step.get("variable_name"):
            ui_helper.store_variable(
                step["variable_name"], result.passed, step.get("scope", "global")
            )
...
class AssertNotEqualsCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_not_equals_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
...
class AssertContainsCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_contains_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
...
class AssertNotContainsCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_not_contains_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
...
class SoftAssertCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"soft_assert_{int(time.time() * 1000)}"
        )
...
class HardAssertCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"hard_assert_{int(time.time() * 1000)}"
        )
        operator = AssertionOperator(step.get("operator", "equals"))
        expected_value = step.get("expected_value", value)
...
class RetryAssertCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"retry_assert_{int(time.time() * 1000)}"
        )
        operator = AssertionOperator(step.get("operator", "equals"))
        expected_value = step.get("expected_value", value)
...
class CheckSoftAssertionsCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        try:
            ...
            if step.get("variable_name"):
                ui_helper.store_variable(
                    step["variable_name"], True, step.get("scope", "global")
                )
            ...
            if step.get("variable_name"):
                ui_helper.store_variable(
                    step["variable_name"], False, step.get("scope", "global")
                )
...
class GetAssertionStatsCommand(BaseAssertCommand):
    ...
    def execute(
        ...
        if step.get("variable_name"):
            ui_helper.store_variable(
                step["variable_name"], stats, step.get("scope", "global")
            )

        logger.info(
            f"断言统计: 总计 {stats['total_assertions']}, 通过 {stats['passed_assertions']}, 失败 {stats['failed_assertions']}"
        )
...
class AssertStartsWithCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_starts_with_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
            else step.get("actual_value")
        )
...
class AssertEndsWithCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_ends_with_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
        actual_value = (
            ui_helper.get_element_text(selector)
...
class AssertMatchesCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_matches_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
        actual_value = (
            ui_helper.get_element_text(selector)
...
class AssertGreaterThanCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_greater_than_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
        actual_value = (
            ui_helper.get_element_text(selector)
...
class AssertLessThanCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_less_than_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
        actual_value = (
            ui_helper.get_element_text(selector)
...
class AssertNotInCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_not_in_{int(time.time() * 1000)}"
        )
        expected_value = step.get("expected_value", value)
        actual_value = (
            ui_helper.get_element_text(selector)
...
class AssertEmptyCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_empty_{int(time.time() * 1000)}"
        )
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
...
class AssertNotEmptyCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_not_empty_{int(time.time() * 1000)}"
        )
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
...
class AssertNullCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_null_{int(time.time() * 1000)}"
        )
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
...
class AssertNotNullCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_not_null_{int(time.time() * 1000)}"
        )
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
...
class AssertTrueCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_true_{int(time.time() * 1000)}"
        )
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
...
class AssertFalseCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_false_{int(time.time() * 1000)}"
        )
        actual_value = (
            ui_helper.get_element_text(selector)
            if selector
...
class AssertLengthCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"assert_length_{int(time.time() * 1000)}"
        )
...
class ConditionalAssertCommand(BaseAssertCommand):
    def execute(
        ...
        # 执行断言
        assertion_id = step.get(
            "assertion_id", f"conditional_assert_{int(time.time() * 1000)}"
        )
        operator = AssertionOperator(step.get("operator", "equals"))
        expected_value = step.get("expected_value", value)
        actual_value = (
    ...
    def execute(
        ...
        for i, assertion in enumerate(assertions):
            try:
                assertion_id = assertion.get(
                    "assertion_id", f"batch_assert_{i}_{int(time.time() * 1000)}"
                )
                ...
                config = self.create_assertion_config(
                    assertion, operator, AssertionType.BATCH
                )
                config.expected_value = expected_value
                result = self.assertion_manager.execute_assertion(
                    assertion_id, actual_value, config
        ...
        if step.get("variable_name"):
            batch_result = {
                ...
                "results": [
                    {"id": r.assertion_id, "passed": r.passed, "message": r.message}
                    for r in results
                ],
            }
            ui_helper.store_variable(
                step["variable_name"], batch_result, step.get("scope", "global")
            )
...
class TimeoutAssertCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"timeout_assert_{int(time.time() * 1000)}"
        )
        operator = AssertionOperator(step.get("operator", "equals"))
        expected_value = step.get("expected_value", value)
        ...
        # 执行超时断言（实际值在超时断言内部获取）
        result = self.assertion_manager.execute_assertion(
            assertion_id, get_actual_value(), config
        )
...
class CustomAssertCommand(BaseAssertCommand):
    def execute(
        ...
        assertion_id = step.get(
            "assertion_id", f"custom_assert_{int(time.time() * 1000)}"
        )

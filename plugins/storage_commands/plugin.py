...
class VariableManager:
    ...
    def set_variable(self, key: str, value: Any, scope: Union[str, StorageScope] = StorageScope.GLOBAL,
    ...
    def get_variable(self, key: str, scope: Union[str, StorageScope] = StorageScope.GLOBAL,
    ...
    def delete_variable(self, key: str, scope: Union[str, StorageScope] = StorageScope.GLOBAL) -> bool:
        """删除变量"""
        if isinstance(scope, str):
            scope = StorageScope(scope)
    ...
    def list_variables(self, scope: Optional[Union[str, StorageScope]] = None) -> Dict[str, Any]:
        """列出变量"""
        if scope is None:
            return {key: entry.value for key, entry in self.variables.items() if not entry.is_expired()}
        
        if isinstance(scope, str):
            scope = StorageScope(scope)
            
        return {key: entry.value for key, entry in self.scoped_variables[scope].items() if not entry.is_expired()}
    ...
    def cleanup_expired(self) -> int:
        ...
        with self.lock:
            for scope in StorageScope:
                expired_keys = [key for key, entry in self.scoped_variables[scope].items() if entry.is_expired()]
    ...
    def _notify_listeners(self, action: str, key: str, value: Any, scope: StorageScope) -> None:
        """通知监听器"""
...
class PersistenceManager:
    ...
    def save(self, key: str, value: Any, scope: StorageScope = StorageScope.GLOBAL,
            ttl: Optional[int] = None, format: SerializationFormat = SerializationFormat.JSON,
    ...
    def load(self, key: str, scope: StorageScope = StorageScope.GLOBAL, default: Any = None) -> Any:
        """加载数据"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
...
class StorageCommandsPlugin:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.persistence_manager = PersistenceManager(
            storage_path=self.config.get('persistence', {}).get('storage_path', 'data/storage')
        )
        ...
        def cleanup_task():
            while True:
                try:
                    ...
                    if total_count > 0:
                        logger.info(f"定期清理完成: 变量({var_count}) 缓存({cache_count}) 持久化({persist_count})")
...
class StoreVariableCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            plugin.variable_manager.set_variable(var_name, var_value, scope, ttl, metadata)
        else:
            # 向后兼容
...
class StoreAttributeCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        # 获取元素属性
        attr_value = ui_helper.get_element_attribute(selector=selector, attribute=attribute)
        
        # 使用插件的变量管理器
        plugin = getattr(ui_helper, 'storage_plugin', None)
        if plugin:
            plugin.variable_manager.set_variable(var_name, attr_value, scope, ttl, metadata)
        else:
            # 向后兼容
...
class SaveElementCountCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        plugin = getattr(ui_helper, 'storage_plugin', None)
        if plugin:
            plugin.variable_manager.set_variable(var_name, str(count), scope, ttl, metadata)
        else:
            # 向后兼容
            if "variable_name" in step:
                ui_helper.store_variable(step["variable_name"], str(count), step.get("scope", "global"))
...
class StoreJsonCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        plugin = getattr(ui_helper, 'storage_plugin', None)
        if plugin:
            plugin.variable_manager.set_variable(var_name, json_data, scope, ttl, metadata)
...
class StoreListCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        plugin = getattr(ui_helper, 'storage_plugin', None)
        if plugin:
            plugin.variable_manager.set_variable(var_name, list_data, scope, ttl, metadata)
...
class StoreExpressionCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        try:
            ...
            plugin = getattr(ui_helper, 'storage_plugin', None)
            if plugin:
                plugin.variable_manager.set_variable(var_name, result, scope, ttl, metadata)
...
class GetVariableCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        plugin = getattr(ui_helper, 'storage_plugin', None)
        if plugin:
            result = plugin.variable_manager.get_variable(var_name, scope, default_value)
            plugin.variable_manager.set_variable(target_var, result, scope)
            logger.debug(f"获取变量 {var_name}={result} -> {target_var} (scope={scope})")
        else:
            raise RuntimeError("存储插件未初始化")
...
class DeleteVariableCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            ...
            logger.debug(f"删除变量 {var_name} (scope={scope}): {'成功' if success else '失败'}")
        else:
            raise RuntimeError("存储插件未初始化")
...
class SavePersistentCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            ...
            format_enum = SerializationFormat(format_str)
            plugin.persistence_manager.save(key, persist_value, scope_enum, ttl, format_enum, metadata)
...
class LoadPersistentCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            ...
            plugin.variable_manager.set_variable(target_var, result, "global")
            logger.debug(f"加载持久化数据 {key}={result} -> {target_var} (scope={scope})")
        else:
            raise RuntimeError("存储插件未初始化")
...
class DeletePersistentCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            ...
            logger.debug(f"删除持久化数据 {key} (scope={scope}): {'成功' if success else '失败'}")
        else:
            raise RuntimeError("存储插件未初始化")
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            ...
            for var_name in variable_names:
                results[var_name] = plugin.variable_manager.get_variable(var_name, scope)
        ...
        if plugin:
            ...
            for var_name in variable_names:
                if plugin.variable_manager.delete_variable(var_name, scope):
                    count += 1
            logger.debug(f"批量删除 {count}/{len(variable_names)} 个变量 (scope={scope})")
        else:
            raise RuntimeError("存储插件未初始化")
...
class StorageStatsCommand(Command):
    ...
    def execute(self, ui_helper, selector: str, value: Any, step: Dict[str, Any]) -> None:
        ...
        if plugin:
            ...
            var_stats = {}
            for scope in StorageScope:
                var_stats[scope.value] = len(plugin.variable_manager.list_variables(scope))
            ...
            # 持久化统计
            persist_stats = {
                'total_keys': len(plugin.persistence_manager.list_keys())
            }

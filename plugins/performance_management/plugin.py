...
class PerformanceMonitor:
    ...
    def record_operation(
        self, operation_name: str, duration: float, context: Dict[str, Any] = None
    ):
        """记录操作性能"""
        with self.lock:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            stats = {
                "resource_usage": {
                    "current": (
                        self.resource_history[-1].__dict__
...
class PerformanceManagementPlugin:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            "optimizer": {"enabled": True, "auto_optimize": False},
        }
...
class RecordPerformanceMetricCommand(Command):
    ...
    def execute(
        ...
        plugin = PerformanceManagementPlugin()
        plugin.monitor.add_metric(
            metric_name, float(metric_value), unit, category, tags
        )
    ...
    def execute(
        ...
        if variable_name:
            ui_helper.store_variable(
                variable_name, cached_value, step.get("scope", "global")
            )
...
class GetOptimizationSuggestionsCommand(Command):
    ...
    def execute(
        ...
        if variable_name:
            ui_helper.store_variable(
                variable_name, suggestions, step.get("scope", "global")
            )
    ...
    def execute(
        ...
        report_data = {
            ...
            "optimization_history": (
                plugin.optimizer.optimization_history if include_history else []
            ),
        }
        ...
        try:
            ...
            if variable_name:
                ui_helper.store_variable(
                    variable_name, report_data, step.get("scope", "global")
                )

...
class DingTalkProvider(NotificationProvider):
    ...
    def send(self, message: NotificationMessage) -> NotificationResult:
        ...
        result = NotificationResult(
            message_id=message.id, provider=self.name, status=NotificationStatus.SENDING
        )
        ...
        try:
            ...
            if message.recipients:
                payload["at"] = {"atMobiles": message.recipients, "isAtAll": False}
            else:
...
class EmailProvider(NotificationProvider):
    ...
    def validate_config(self) -> bool:
        """验证配置"""
        return bool(
            self.smtp_server and self.username and self.password and self.from_email
        )
...
class NotificationTemplate:
    ...
    def __init__(
        self, name: str, template: str, type: NotificationType = NotificationType.TEXT
    ):
        self.name = name
...
class NotificationManager:
    ...
    def send_notification(
        self, message: NotificationMessage, providers: Optional[List[str]] = None
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            success_count = sum(
                1 for r in self.history if r.status == NotificationStatus.SUCCESS
            )
...
class NotificationPlugin:
    ...
    def _register_commands(self) -> None:
        ...
        CommandFactory.register("send_template_notification")(
            SendTemplateNotificationCommand
        )
        ...
        CommandFactory.register("send_conditional_notification")(
            SendConditionalNotificationCommand
        )

        # 管理命令
        CommandFactory.register("get_notification_status")(GetNotificationStatusCommand)
        CommandFactory.register("get_notification_history")(
            GetNotificationHistoryCommand
        )
        CommandFactory.register("get_notification_stats")(GetNotificationStatsCommand)
        CommandFactory.register("cleanup_notification_history")(
            CleanupNotificationHistoryCommand
        )
...
class SendDingTalkCommand(Command):
    ...
    def execute(
        ...
        logger.info(
            f"发送钉钉通知完成: {results[0].status.value if results else '无结果'}"
        )
...
class SendEmailCommand(Command):
    ...
    def execute(
        ...
        logger.info(
            f"发送邮件通知完成: {results[0].status.value if results else '无结果'}"
        )
...
class SendWebhookCommand(Command):
    ...
    def execute(
        ...
        logger.info(
            f"发送Webhook通知完成: {results[0].status.value if results else '无结果'}"
        )
...
class SendSlackCommand(Command):
    ...
    def execute(
        ...
        logger.info(
            f"发送Slack通知完成: {results[0].status.value if results else '无结果'}"
        )
...
class SendBatchNotificationCommand(Command):
    ...
    def execute(
        ...
        logger.info(
            f"发送批量通知完成: {len(notifications)} 条通知, {len(all_results)} 个结果"
        )
...
class GetNotificationStatsCommand(Command):
    ...
    def execute(
        ...
        logger.info(
            f"获取通知统计: 总计 {stats['total']} 条，成功率 {stats['success_rate']:.2%}"
        )

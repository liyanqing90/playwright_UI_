...
def pytest_addoption(parser):
    """添加命令行参数支持"""
    parser.addoption(
        "--group", action="store", default=None, help="指定要运行的测试用例组"
    )
...
def sort_items_by_dependency_and_priority(items):
    ...
    # 创建依赖图
    dependency_graph = {
        item.nodeid: set(test_dependencies.get(item.nodeid, [])) for item in items
    }
    ...
    # 按优先级排序起始节点
    start_nodes = sorted(
        all_tests, key=lambda x: get_test_priority(items_dict.get(x)), reverse=True
    )

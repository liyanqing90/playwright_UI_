...
class CommandProtocol(Protocol):
    ...
    def execute(
        self, ui_helper: Any, selector: str, value: Any, step: StepDefinition
    ) -> Any:
        """执行命令
...
class AssertionProtocol(Protocol):
    ...
    def assert_contains(
        self, container: Any, item: Any, message: Optional[str] = None
    ) -> AssertionResult:
        """包含断言"""
        ...

    def assert_visible(
        self, selector: str, message: Optional[str] = None
    ) -> AssertionResult:
        """可见性断言"""
        ...

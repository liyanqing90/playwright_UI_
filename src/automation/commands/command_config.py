...
class CommandConfigManager:
            ...
            if os.path.exists(self.config_file):
                ...
                if 'commands' in data:
                    for name, config_data in data['commands'].items():
                        ...
                        self.command_configs[name] = CommandConfig.from_dict(config_data)
    ...
    def set_command_config(self, command_name: str, config: Union[CommandConfig, Dict[str, Any]]):
        """设置命令配置"""
    ...
    def get_commands_by_tag(self, tag: str) -> List[str]:
        """根据标签获取命令列表"""
        return [
            name for name, config in self.command_configs.items()
    ...
    def get_commands_by_priority(self, min_priority: int = 0) -> List[str]:
        """根据优先级获取命令列表"""
        commands = [
            (name, config.priority) for name, config in self.command_configs.items()

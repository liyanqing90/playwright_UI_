...
class CommandExecutionError(Exception):
    ...
    def __init__(
        self, command_name: str, message: str, original_error: Exception = None
    ):
...
class CommandExecutor:
    ...
    def __init__(self, max_workers: int = None):
        self.max_workers = (
            max_workers or config_manager.get_global_config().max_concurrent_commands
        )
    ...
    def remove_hook(self, hook_type: str, hook_func: Callable):
        """移除执行钩子"""
        if (
            hook_type in self.execution_hooks
            and hook_func in self.execution_hooks[hook_type]
        ):
    ...
    def _execute_with_features(self, command: Command, *args, **kwargs) -> Any:
        ...
        for attempt in range(retry_count + 1):
            try:
                if attempt > 0:
                    logger.info(
                        f"Retrying command {command_name}, attempt {attempt + 1}/{retry_count + 1}"
                    )
                ...
                last_error = e
                logger.warning(
                    f"Command {command_name} failed on attempt {attempt + 1}: {e}"
                )
        ...
        raise CommandExecutionError(
            command_name, "Unexpected execution path", last_error
        )

    def _execute_with_timeout(
        self, command: Command, timeout: float, *args, **kwargs
    ) -> Any:
        """带超时的命令执行"""
        try:
            future = self.thread_pool.submit(
                command.execute_with_monitoring, *args, **kwargs
            )
            ...
            raise CommandTimeoutError(
                command.name, f"Command timed out after {timeout}s"
            )
        except Exception as e:
            raise e
    ...
    async def _execute_async_with_features(
        ...
        for attempt in range(retry_count + 1):
            try:
                if attempt > 0:
                    logger.info(
                        f"Retrying async command {command_name}, attempt {attempt + 1}/{retry_count + 1}"
                    )
                    self._call_hooks("on_retry", command_name, attempt, *args, **kwargs)
                    await asyncio.sleep(retry_delay)

                result = await self._execute_async_with_timeout(
                    command, timeout, *args, **kwargs
                )
                ...
                last_error = e
                logger.warning(
                    f"Async command {command_name} failed on attempt {attempt + 1}: {e}"
                )
        ...
        try:
            with command_monitor.monitor_command(command.name):
                result = await asyncio.wait_for(
                    command.execute_async(*args, **kwargs), timeout=timeout
                )
            return result
        except asyncio.TimeoutError:
            self._call_hooks("on_timeout", command.name, timeout, *args, **kwargs)
            raise CommandTimeoutError(
                command.name, f"Async command timed out after {timeout}s"
            )
        except Exception as e:
            raise e

    def execute_batch(
        self, commands: List[Dict[str, Any]], parallel: bool = False
    ) -> List[Any]:
        """批量执行命令"""
        if parallel:
    ...
    def _execute_batch_parallel(self, commands: List[Dict[str, Any]]) -> List[Any]:
        ...
        for cmd_info in commands:
            ...
            future = self.thread_pool.submit(
                self.execute_command, action, *args, **kwargs
            )
    ...
    async def execute_batch_async(
        self, commands: List[Dict[str, Any]], parallel: bool = True
    ) -> List[Any]:
        """异步批量执行命令"""
        if parallel:
    ...
    async def _execute_batch_async_sequential(
    ...
    async def _execute_batch_async_parallel(
        self, commands: List[Dict[str, Any]]
    ) -> List[Any]:
        """异步并行执行命令批次"""
        tasks = []

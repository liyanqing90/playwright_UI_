...
class CommandCLI:
    ...
    def create_parser(self) -> argparse.ArgumentParser:
        ...
        list_parser.add_argument(
            "--enabled-only", action="store_true", help="Show only enabled commands"
        )
        ...
        set_config_parser.add_argument(
            "--retry-delay", type=float, help="Retry delay in seconds"
        )
        set_config_parser.add_argument(
            "--enabled", type=bool, help="Enable/disable command"
        )
        ...
        import_config_parser.add_argument(
            "--merge", action="store_true", help="Merge with existing config"
        )
        ...
        # 显示指标
        metrics_parser = monitor_subparsers.add_parser(
            "metrics", help="Show command metrics"
        )
        ...
        # 重置指标
        reset_metrics_parser = monitor_subparsers.add_parser(
            "reset", help="Reset metrics"
        )
        reset_metrics_parser.add_argument(
            "--command", help="Specific command name (all if not specified)"
        )
        ...
        discover_parser.add_argument(
            "--force", action="store_true", help="Force rediscovery"
        )
    ...
    def list_commands(self, args):
        ...
        if args.enabled_only:
            commands = [
                cmd
                for cmd in commands
                if config_manager.get_command_config(cmd).enabled
            ]
        ...
        if args.tag:
            commands = [
                cmd
                for cmd in commands
                if args.tag in config_manager.get_command_config(cmd).tags
            ]
        ...
        if args.format == "json":
            ...
            print(
                f"{'Command':<20} {'Enabled':<8} {'Timeout':<8} {'Retry':<6} {'Priority':<8} {'Tags'}"
            )
            print("-" * 80)
            for cmd in commands:
                config = config_manager.get_command_config(cmd)
                ...
                print(
                    f"{cmd:<20} {config.enabled!s:<8} {config.timeout:<8.1f} {config.retry_count:<6} {config.priority:<8} {tags_str}"
                )
    ...
    def show_metrics(self, args):
        ...
        if args.format == "json":
            ...
            print(
                f"{'Command':<20} {'Count':<8} {'Total(s)':<10} {'Avg(s)':<8} {'Min(s)':<8} {'Max(s)':<8} {'Errors':<8}"
            )
            print("-" * 90)
            for name, metric in metrics.items():
                min_time = metric.min_time if metric.min_time != float("inf") else 0
                print(
                    f"{name:<20} {metric.execution_count:<8} {metric.total_time:<10.2f} {metric.avg_time:<8.2f} {min_time:<8.2f} {metric.max_time:<8.2f} {metric.error_count:<8}"
                )
    ...
    def list_plugins(self):
        ...
        for plugin in plugins:
            ...
            print(
                f"{plugin:<20} {info.get('version', 'N/A'):<10} {info.get('enabled', False)!s:<8} {commands_count:<10}"
            )

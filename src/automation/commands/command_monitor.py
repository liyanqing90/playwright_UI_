...
class CommandMonitor:
    ...
    def __init__(self):
        self.metrics: Dict[str, CommandMetrics] = defaultdict(
            lambda: CommandMetrics("")
        )
        self.enabled = True
    ...
    def _record_execution(
        self, command_name: str, execution_time: float, success: bool
    ):
        """记录命令执行"""
        with self._lock:
    ...
    def get_metrics(self, command_name: str = None) -> Dict[str, CommandMetrics]:
        ...
        with self._lock:
            if command_name:
                return {
                    command_name: self.metrics.get(
                        command_name, CommandMetrics(command_name)
                    )
                }
    ...
    def get_top_commands(
        self, limit: int = 10, sort_by: str = "total_time"
    ) -> List[CommandMetrics]:
        """获取排行榜"""
        with self._lock:
        ...
        with self._lock:
            ...
            if slow_commands:
                ...
                for cmd in slow_commands[:5]:
                    lines.append(
                        f"{cmd.command_name}: {cmd.avg_time:.2f}s avg, {cmd.execution_count} executions"
                    )
                lines.append("")
            ...
            if error_commands:
                ...
                for cmd in error_commands[:5]:
                    lines.append(
                        f"{cmd.command_name}: {cmd.get_error_rate():.2%} error rate, {cmd.error_count}/{cmd.execution_count} errors"
                    )
                lines.append("")
            ...
            for cmd in top_commands:
                lines.append(
                    f"{cmd.command_name}: {cmd.execution_count} executions, {cmd.avg_time:.2f}s avg"
                )

...
class Command(ABC):
    ...
    def __init__(self):
        self.config: CommandConfig = config_manager.get_command_config(self.__class__.__name__)
    ...
    def auto_discover_commands(cls, package_name: str = None):
        ...
        try:
            ...
            for importer, modname, ispkg in pkgutil.iter_modules(package_path):
                if not ispkg and modname != 'base_command' and not modname.startswith('_'):
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                if (issubclass(obj, Command) and 
                    ...
                    # 检查是否已经通过装饰器注册
                    if not any(obj == cmd_class for cmd_class in cls._commands.values()):
                        # 尝试从类名推断操作类型
                        action_type = name.replace('Command', '').lower()
                        cls._commands[action_type] = obj
                        logger.info(f"Auto-registered command from module {module_name}: {action_type} -> {name}")

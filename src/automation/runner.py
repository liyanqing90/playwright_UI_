...
class TestCaseGenerator(pytest.Item):
    ...
    def _create_test_function(self, case: dict):
        ...
        # 确保case_data是列表类型，防止None值导致迭代错误
        if case_data is None:
            logger.warning(f"用例 {case_name} 对应的测试数据不存在，使用空列表替代")
            case_data = []
        else:
            case_data = (
                [case_data]
        ...
        def _test_function_wrapper_for_case(
            page, ui_helper, get_test_name, **kwargs
        ):  # 重命名闭包函数
            self.execute_test(
    ...
    def execute_test(
        ...
        executor.execute_test_case(page, ui_helper, test_name=test_name)

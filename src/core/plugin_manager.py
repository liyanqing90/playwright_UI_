...
from .interfaces.plugin_interface import (
    PluginInterface, PluginMetadata, PluginStatus, PluginLoadError, PluginInitializationError, PluginDependencyError,
...
class PluginManager:
                ...
                if plugin_dir.is_dir() and not plugin_dir.name.startswith('.'):
                    ...
                    if metadata_file.exists():
                        try:
                            ...
                            self.logger.error(f"Failed to load metadata for {plugin_dir.name}: {e}")
    ...
    def load_plugin(self, metadata: PluginMetadata, plugin_directory: str) -> bool:
        ...
        try:
            ...
            if not isinstance(plugin_instance, PluginInterface):
                raise PluginLoadError(f"Plugin {metadata.name} does not implement PluginInterface")
            ...
            self.logger.info(f"Successfully loaded plugin: {metadata.name} ({metadata.load_time:.3f}s)")
            return True
    ...
    def initialize_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            if not self._check_dependencies(metadata):
                raise PluginDependencyError(f"Dependencies not satisfied for {plugin_name}")
    ...
    def _check_dependencies(self, metadata: PluginMetadata) -> bool:
        ...
        for dependency in metadata.dependencies:
            if dependency not in self.plugins:
                self.logger.error(f"Dependency {dependency} not found for plugin {metadata.name}")
                return False
            
            dep_metadata = self.plugin_metadata[dependency]
            if dep_metadata.status != PluginStatus.ACTIVE:
                self.logger.error(f"Dependency {dependency} not active for plugin {metadata.name}")
                return False
        ...
        for priority in sorted(priority_groups.keys(), key=lambda x: x.value):
            ...
            while remaining:
                ...
                for name, plugin in list(remaining.items()):
                    # 检查依赖是否已解决
                    deps_resolved = all(dep in resolved or dep in load_order 
                ...
                if not progress and remaining:
                    # 循环依赖或缺失依赖
                    self.logger.error(f"Circular dependency or missing dependency detected: {list(remaining.keys())}")
                    # 强制添加剩余插件
    ...
    def load_all_plugins(self) -> Dict[str, bool]:
        ...
        for plugin_name in self.load_order:
            plugin_metadata = next((p for p in discovered_plugins if p.name == plugin_name), None)
        ...
        self.logger.info(f"Plugin loading completed. Success: {sum(results.values())}, Failed: {len(results) - sum(results.values())}")
        return results
    ...
    def list_plugins(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, metadata in self.plugin_metadata.items():
            plugin_info[name] = {
                ...
                'health': self.plugins[name].get_health_status() if name in self.plugins else None
            }
        return plugin_info
    ...
    def restart_plugin(self, plugin_name: str) -> bool:
        ...
        if self.unload_plugin(plugin_name):
            ...
            if metadata:
                ...
                if plugin_directory:
                    return (self.load_plugin(metadata, plugin_directory) and 

...
class ErrorDeduplicationManager:
    ...
    def __init__(self, config_file: str = None):
        ...
        # 错误分类规则
        self.custom_patterns = self.config.get("advanced", {}).get(
            "custom_error_patterns", []
        )
        self.ignore_patterns = self.config.get("advanced", {}).get(
            "ignore_patterns", []
        )

        logger.info(
            f"错误去重管理器已初始化 - 启用: {self.enabled}, 时间窗口: {self.time_window}s, 最大重复: {self.max_same_error_count}次"
        )
    ...
    def _generate_error_hash(
        self, error_message: str, error_type: str = None, selector: str = None
    ) -> str:
        """生成错误哈希值"""
    ...
    def _should_ignore_error(self, error_message: str) -> bool:
        ...
        for pattern in self.ignore_patterns:
            if re.search(pattern, error_message, re.IGNORECASE):
                logger.debug(
                    f"忽略错误（匹配忽略规则）: {self._truncate_message(error_message)}"
                )
                return True
        return False

    def _get_max_count_for_error(
        self, error_message: str, error_type: str = None
    ) -> int:
        """获取特定错误的最大计数"""
        # 检查自定义模式
        ...
        if error_type:
            ...
            if type_key in error_types_config:
                return error_types_config[type_key].get(
                    "max_count", self.max_same_error_count
                )
    ...
    def get_error_statistics(self) -> Dict:
        ...
        # 统计重复错误
        repeated_errors = sum(
            1 for record in self.error_records.values() if record.count > 1
        )

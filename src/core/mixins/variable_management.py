...
class VariableManagementMixin:
    ...
    def get_all_variables(self) -> Dict[str, Any]:
        ...
        with allure.step("获取所有变量"):
            ...
            allure.attach(
                var_info, name="所有变量", attachment_type=allure.attachment_type.TEXT
            )
    ...
    def store_element_attribute_as_variable(
        self, selector: str, attribute: str, variable_name: str
    ):
        """
        从元素属性存储变量
    ...
    def increment_variable(self, variable_name: str, increment: int = 1):
        ...
        with allure.step(f"增加变量值: {variable_name}"):
            ...
            try:
                ...
                logger.info(
                    f"变量值增加成功: {variable_name} {current_value} -> {new_value}"
                )
                ...
                logger.error(
                    f"变量值增加失败: {variable_name} 不是数值类型 ({current_value})"
                )
    ...
    def format_variable(
        ...
        with allure.step(f"格式化变量: {variable_name}"):
            ...
            try:
                ...
                if self.variable_manager:
                    resolved_format = (
                        self.variable_manager.replace_variables_refactored(
                            format_string
                        )
                    )
                else:
                ...
                logger.info(
                    f"变量格式化成功: {variable_name} ({original_value}) -> {target_var} ({formatted_value})"
                )

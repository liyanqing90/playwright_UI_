...
class ErrorReporter:
            ...
            error_details.append(
                {
                    ...
                    "first_occurrence": datetime.fromtimestamp(
                        record.first_occurrence
                    ).isoformat(),
                    "last_occurrence": datetime.fromtimestamp(
                        record.last_occurrence
                    ).isoformat(),
                    "count": record.count,
                    "is_suppressed": error_hash
                    in error_dedup_manager.suppressed_errors,
                }
            )
    ...
    def generate_summary_report(self) -> str:
        ...
        lines.append(
            f"  • 最近{stats['time_window_minutes']:.1f}分钟内的错误: {stats['recent_errors']}"
        )
        ...
        if error_details:
            ...
            for i, error in enumerate(error_details[:10], 1):
                ...
                lines.append(
                    f"      {self._truncate_message(error['error_message'], 80)}"
                )
...
def generate_final_error_report():
    ...
    try:
        ...
        return {"detailed_report": report_file, "summary_report": summary_file}

...
class PerformanceAnalysisEngine:
    ...
    def analyze_performance_data(
        ...
        analysis_result = {
            ...
            "summary": self._generate_analysis_summary(
                bottlenecks, trends, suggestions
            ),
        }
    ...
    def identify_bottlenecks(
        self, current_metrics: Dict[str, Any], baseline_data: Dict[str, Any]
    ) -> List[Bottleneck]:
        """识别性能瓶颈
        ...
        if page_load_metrics:
            bottlenecks.extend(
                self._analyze_page_load_bottlenecks(page_load_metrics, baseline_data)
            )
        ...
        if web_vitals:
            bottlenecks.extend(
                self._analyze_web_vitals_bottlenecks(web_vitals, baseline_data)
            )
    ...
    def _analyze_page_load_bottlenecks(
        self, page_metrics: Dict[str, Any], baseline_data: Dict[str, Any]
    ) -> List[Bottleneck]:
        """分析页面加载瓶颈"""
        bottlenecks = []
    ...
    def _analyze_web_vitals_bottlenecks(
        self, web_vitals: Dict[str, Any], baseline_data: Dict[str, Any]
    ) -> List[Bottleneck]:
        """分析Web Vitals瓶颈"""
        bottlenecks = []
    ...
    def _analyze_action_bottlenecks(
    ...
    def analyze_root_causes(
        self, bottlenecks: List[Bottleneck], current_metrics: Dict[str, Any]
    ) -> Dict[str, List[str]]:
        """分析根因
        ...
        for bottleneck in bottlenecks:
            ...
            if bottleneck.type == BottleneckType.NETWORK:
                causes.extend(
                    ["网络连接质量差", "服务器响应时间长", "DNS解析慢", "CDN配置问题"]
                )
                ...
                causes.extend(
                    ["图片未优化", "JavaScript/CSS文件未压缩", "缺少资源缓存策略"]
                )
                ...
                causes.extend(
                    ["JavaScript执行时间长", "主线程阻塞", "内存泄漏", "未优化的算法"]
                )
        ...
        return {
            ...
            "priority_suggestions": len(
                [
                    ...
                    if s.priority in [SeverityLevel.CRITICAL, SeverityLevel.HIGH]
                ]
            ),

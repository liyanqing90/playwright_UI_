...
class PerformanceBaseline:
    ...
    def __init__(
        ...
        self.baseline_file = (
            Path(baseline_file) if baseline_file else Path("performance_baseline.json")
        )
    ...
    def add_metric(
        ...
        if len(self.baselines[metric_name]) > self.config.max_samples:
            self.baselines[metric_name] = self.baselines[metric_name][
                -self.config.max_samples :
            ]

        logger.debug("添加性能指标", metric_name=metric_name, value=value, unit=unit)
    ...
    def compare_with_baseline(
        ...
        if (
            ...
            or len(self.baselines[metric_name]) < self.config.min_samples
        ):
            return {
    ...
    def get_baseline_stats(self, metric_name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "unit": (
                self.baselines[metric_name][0].unit
                if self.baselines[metric_name]
                else "unknown"
            ),
        }
    ...
    def _cleanup_old_data(self) -> None:
        ...
        for metric_name in list(self.baselines.keys()):
            ...
            self.baselines[metric_name] = [
                m
                for m in self.baselines[metric_name]
            ...
            if cleaned_count > 0:
                logger.debug(
                    "清理过期数据", metric_name=metric_name, cleaned_count=cleaned_count
                )

    def _generate_comparison_message(
        self, result: ComparisonResult, diff_percentage: float
    ) -> str:
        """生成比较结果消息"""
        if result == ComparisonResult.BETTER:

...
class PerformanceDashboard:
        ...
        if page_load:
            formatted_metrics["page_load"] = {
                "load_time": {
                    ...
                    "unit": "ms",
                    "status": self._get_metric_status("load_time_ms", page_load.get("load_time_ms", 0))
                },
                "dom_ready": {
                    "value": page_load.get("dom_ready_ms", 0),
                    "unit": "ms",
                    "status": self._get_metric_status("dom_ready_ms", page_load.get("dom_ready_ms", 0))
                },
                "response_time": {
                    "value": page_load.get("response_time_ms", 0),
                    "unit": "ms",
                    "status": self._get_metric_status("response_time_ms", page_load.get("response_time_ms", 0))
                }
            }
    ...
    def _generate_chart_data(self) -> List[Dict[str, Any]]:
        ...
        # Web Vitals趋势图
        lcp_chart = self._create_time_series_chart(
            "LCP趋势",
            "lcp",
            "web_vitals",
            "ms"
    ...
    def _create_time_series_chart(self, 
                                title: str,
                                metric_name: str,
                                category: str,
    ...
    def _create_performance_distribution_chart(self) -> Optional[Dict[str, Any]]:
        ...
        data_points = [
            {"label": "优秀", "value": status_counts["excellent"], "color": self.config.chart_colors["excellent"]},
            {"label": "良好", "value": status_counts["good"], "color": self.config.chart_colors["good"]},
            {"label": "一般", "value": status_counts["fair"], "color": self.config.chart_colors["fair"]},
            {"label": "较差", "value": status_counts["poor"], "color": self.config.chart_colors["poor"]}
        ]
    ...
    def _get_optimization_suggestions(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """获取优化建议"""
        # 这里可以集成分析引擎的优化建议
        analysis_result = data.get("analysis_result", {})
        ...
        try:
            ...
            if format_type.lower() == "json":
                ...
                with open(file_path, 'w', newline='', encoding='utf-8') as f:
                    ...
                    # 写入表头
                    writer.writerow(["时间戳", "页面加载时间(ms)", "LCP(ms)", "FID(ms)", "CLS"])

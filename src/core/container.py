...
from .interfaces.service_interface import (
    ServiceInterface,
    ConfigurableService,
    LifecycleService,
)
...
class ServiceContainer:
    ...
    def __init__(
        ...
        self._circular_dependency_detection = global_config.container.get(
        ...
        try:
            ...
            if scope == ServiceScope.SINGLETON:
                self.register_implementation(
                    interface_type, service_class, singleton=True
                )
            elif scope == ServiceScope.TRANSIENT:
                self.register_implementation(
                    interface_type, service_class, singleton=False
                )
            ...
            # 对于可选服务，记录警告但不中断整个系统
            if (
                "performance" in service_config.name.lower()
                or "optimization" in service_config.name.lower()
            ):
                logger.warning(
                    f"可选服务 {service_config.name} 注册失败，系统将继续运行"
                )
                return
            raise
    ...
    def _import_class(self, class_path: str) -> Type:
        ...
        try:
            ...
            # 对于可选服务，记录警告但不中断整个系统
            if (
                "performance" in class_path.lower()
                or "optimization" in class_path.lower()
            ):
    ...
    def register_implementation(
        ...
        logger.debug(
            f"已注册服务: {service_name} -> {implementation.__name__} (单例: {singleton})"
        )
        return self
    ...
    def get_config_value(
        self, service_name: str, config_key: str, default: Any = None
    ) -> Any:
        """获取服务配置值
                    ...
                    if param.annotation != inspect.Parameter.empty:
                        try:
                            ...
                            if param.default != inspect.Parameter.empty:
                                ...
                                logger.warning(
                                    f"无法解析依赖 {param_name}: {param.annotation}"
                                )

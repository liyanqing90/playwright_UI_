...
class PluginManager:
    ...
    def __init__(self, plugin_dir: Path = None):
        ...
        try:
            ...
            # 初始化核心插件管理器
            self.core_manager = CorePluginManager(self.service_container, self.core_logger)
    ...
    def discover_plugins(self) -> List[PluginInfo]:
        ...
        if self.core_manager:
            try:
                ...
                logger.warning(f"Core plugin discovery failed, falling back to legacy: {e}")
            ...
            try:
                ...
                if self.core_manager.load_plugin(metadata, str(self.plugin_dir)):
                    if self.core_manager.initialize_plugin(plugin_name):
                        ...
                        logger.info(f"Successfully loaded plugin via core manager: {plugin_name}")
                        return True
                
            except Exception as e:
                logger.warning(f"Core plugin loading failed, falling back to legacy: {e}")
        ...
        if self.core_manager and self.core_manager.get_plugin(plugin_name):
            try:
                if self.core_manager.unload_plugin(plugin_name):
                    ...
                    logger.info(f"Successfully unloaded plugin via core manager: {plugin_name}")
                    return True
            except Exception as e:
                logger.warning(f"Core plugin unloading failed, falling back to legacy: {e}")

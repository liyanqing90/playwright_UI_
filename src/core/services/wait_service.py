...
class WaitOperations(Protocol):
    ...
    def wait_for_url(
        self, page: Page, url_pattern: str, timeout: Optional[int] = DEFAULT_TIMEOUT
    ) -> None:
        """等待URL匹配"""
        ...

    def wait_for_function(
        self, page: Page, expression: str, timeout: Optional[int] = DEFAULT_TIMEOUT
    ) -> Any:
        """等待JavaScript表达式返回真值"""
        ...
...
class WaitService(BaseService):
    ...
    @operation_decorator(operation_type="等待元素状态", auto_screenshot=False)
    def wait_for_element(
        self,
        page: Page,
        selector: str,
    ...
    @operation_decorator(operation_type="等待文本内容", auto_screenshot=False)
    def wait_for_text(
        self,
        page: Page,
        selector: str,
        ...
        page.wait_for_function(lambda: check_text(), timeout=timeout)
    ...
    def wait_for_function(
        ...
        try:
            self._log_operation(
                "wait_for_function", f"expression: {expression[:100]}..."
            )
    ...
    def wait_for_selector_count(
        ...
        try:
            self._log_operation(
                "wait_for_selector_count", f"selector: {selector}, count: {count}"
            )
    ...
    def wait_for_network_idle(
        self, page: Page, timeout: Optional[int] = DEFAULT_TIMEOUT
    ) -> None:
        """等待网络空闲

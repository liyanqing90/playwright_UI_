...
class PageOperations(Protocol):
    ...
    def find_and_click(self, selector: str, timeout: Optional[int] = None) -> None:
        """查找元素并点击"""
        ...
    
    def find_and_fill(self, selector: str, value: str, timeout: Optional[int] = None) -> None:
        """查找元素并填充内容"""
        ...
    
    def wait_and_assert_visible(self, selector: str, timeout: Optional[int] = None) -> None:
        """等待元素可见并断言"""
        ...
    
    def wait_and_assert_text(self, selector: str, expected_text: str, timeout: Optional[int] = None) -> None:
        """等待元素并断言文本内容"""
        ...
...
class PageService(BaseService):
    ...
    def __init__(self, 
        ...
        # 配置选项
        self.auto_wait = self.config.get('auto_wait', True)
    ...
    def navigate_and_wait(self, url: str, wait_for: str = "networkidle") -> None:
        ...
        try:
            ...
            if self.auto_wait:
                self.wait_service.wait_for_load_state(wait_for)
    ...
    def find_and_click(self, selector: str, timeout: Optional[int] = None) -> None:
        ...
        try:
            if self.auto_wait:
                self.wait_service.wait_for_element_visible(selector, timeout)
    ...
    def find_and_fill(self, selector: str, value: str, timeout: Optional[int] = None) -> None:
        ...
        try:
            if self.auto_wait:
                self.wait_service.wait_for_element_visible(selector, timeout)
            self.element_service.fill(selector, value)
        except Exception as e:
            if self.screenshot_on_failure:
    ...
    def wait_and_assert_visible(self, selector: str, timeout: Optional[int] = None) -> None:
        """等待元素可见并断言"""
        self.wait_service.wait_for_element_visible(selector, timeout)
        self.assertion_service.assert_element_visible(selector)
    
    def wait_and_assert_text(self, selector: str, expected_text: str, timeout: Optional[int] = None) -> None:
        """等待元素并断言文本内容"""
        self.wait_service.wait_for_element_visible(selector, timeout)
        self.assertion_service.assert_element_text(selector, expected_text)

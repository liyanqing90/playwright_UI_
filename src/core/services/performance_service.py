...
class PerformanceOperations(Protocol):
    ...
    def measure_action_time(
        self, action_name: str, action_func, *args, **kwargs
    ) -> Dict[str, Any]:
        """测量操作执行时间"""
        ...
...
class PerformanceService:
    ...
    def __init__(
        ...
        self.baseline = PerformanceBaseline(
            baseline_file=baseline_file, config=baseline_config or BaselineConfig()
        )
    ...
    def stop_monitoring(self) -> Dict[str, Any]:
        ...
        logger.info(
            "停止性能监控", test_name=self.current_test, total_time_ms=total_time * 1000
        )
    ...
    def measure_page_load(self, url: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            self.dashboard.record_metric(
                "dom_ready_time", metrics["dom_ready_ms"], {"url": url}
            )
            self.dashboard.record_metric(
                "resource_count", metrics["resource_count"], {"url": url}
            )
    ...
    def measure_action_time(
        ...
        try:
            ...
            # 记录到仪表板
            self.dashboard.record_timer(
                "action_time", execution_time, {"action": action_name}
            )
    ...
    def _generate_recommendations(self, baseline_report: Dict[str, Any]) -> List[str]:
        ...
        try:
            # 使用分析引擎生成智能建议
            analysis_result = self.analysis_engine.analyze_performance_data(
                self.metrics
            )
            ...
            # 合并智能分析建议
            all_recommendations = basic_recommendations + analysis_result.get(
                "recommendations", []
            )
    ...
    def _generate_basic_recommendations(
        self, baseline_report: Dict[str, Any]
    ) -> List[str]:
        """生成基础性能优化建议
            ...
            if comparison.get("result") == "worse":
                ...
                if diff > 20:
                    recommendations.append(
                        f"{metric_name} 性能下降 {diff:.1f}%，需要重点关注"
                    )
                elif "load_time" in metric_name:
                    recommendations.append(
                        "页面加载时间增加，建议检查网络请求和资源大小"
                    )
                elif "action" in metric_name:
                    recommendations.append(
                        "用户操作响应时间增加，建议检查JavaScript执行效率"
                    )
    ...
    async def suggest_optimizations(
        ...
        try:
            ...
            # 记录到仪表盘
            self.dashboard.record_metric(
                "suggestions_generated", len(serializable_suggestions)
            )
    ...
    async def auto_optimize_timeouts(
        self, data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """自动优化超时设置
        ...
        try:
            ...
            # 记录到仪表盘
            self.dashboard.increment_counter(
                "timeout_optimizations", len(optimization_results)
            )
            success_count = sum(
                1 for r in optimization_results if r.get("success", False)
            )
            self.dashboard.record_metric(
                "optimization_success_rate",
                (
                    success_count / len(optimization_results)

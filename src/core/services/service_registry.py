...
class CoreServiceRegistry:
    ...
    def register_core_services(container: ServiceContainer) -> None:
        ...
        # 注册基础服务（无依赖）
        container.register_implementation(VariableService, VariableService, singleton=True)
        ...
        container.register_factory(
            ...
            lambda: ElementService(
                ...
                variable_manager=container.resolve(VariableService) if container.is_registered(VariableService) else None
            ),
            singleton=True
        )
        
        # 注册导航服务
        container.register_implementation(NavigationService, NavigationService, singleton=True)
        
        # 注册等待服务
        container.register_implementation(WaitService, WaitService, singleton=True)
        
        # 注册断言服务
        container.register_implementation(AssertionService, AssertionService, singleton=True)

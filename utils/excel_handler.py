...
def _is_new_case(case_name: str) -> bool:
    ...
    return bool(
        case_name
        and not pd.isna(case_name)
        and case_name.lower() not in ["nan", "none", "null"]
    )
...
def _is_case_skipped(row: pd.Series) -> bool:
    ...
    return not pd.isna(skip_value) and str(skip_value).strip().lower() in [
        "是",
        "y",
        "yes",
        "true",
...
def _create_test_case(
    case: ExcelTestCase, steps: List[Dict[str, str]]
) -> ExcelTestCase:
    """创建完整的测试用例"""
    case.steps = steps
    return case
...
class ExcelHandler:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                selector = (
                    ...
                    if selector_type and selector_type not in ["nan", "css selector"]
                    else selector_value
                )
    ...
    def _read_test_cases(
        self, excel_path: str, sheet_name: str = None
    ) -> List[ExcelTestCase]:
        """读取Excel中的测试用例"""
        try:
    ...
    def _create_step(
        self, row: pd.Series, current_case: ExcelTestCase
        ...
        return {
            k: ("" if pd.isna(v) or str(v).lower() in ["nan", "none", "null"] else v)

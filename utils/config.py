...
class ProjectManager:
    ...
    def get_project_config(self, project_name: str) -> dict:
        ...
        if project_name not in self._projects_cache:
            raise ValueError(
                f"Project '{project_name}' not found. Available projects: {self.get_available_projects()}"
            )
        ...
        try:
            ...
            for field in required_fields:
                if field not in config:
                    logger.error(
                        f"Project '{project_name}' missing required field: {field}"
                    )
                    return False
            ...
            if not isinstance(environments, dict) or not environments:
                logger.error(
                    f"Project '{project_name}' has invalid environments configuration"
                )
                return False
            ...
            if not test_dir.exists():
                logger.warning(
                    f"Test directory '{test_dir}' does not exist for project '{project_name}'"
                )

...
插件系统的核心问题是**缺少初始化调用**。虽然 `PluginManager` 类实现完整，但在系统启动时没有调用
`plugin_manager.load_all_plugins()` 方法。
...
插件系统修复的核心是**添加系统初始化调用**，推荐在 `conftest.py` 的 `pytest_sessionstart`
函数中初始化插件系统。这个方案风险最小，实施最简单，且与现有架构最兼容。
